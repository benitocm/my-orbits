...
def test_elliptical():
    ...
    assert np.linalg.norm(h_xyz) == approx (0.02769576586418702,abs=ABS)
...
def test_hiperbolical():
    ...
    assert np.linalg.norm(h_xyz) == approx (0.04425936532200518,abs=ABS)
...
def test_parabolical():
    ...
    assert np.linalg.norm(h_xyz) == approx (0.0383352619607468,abs=ABS)
