...
def _elliptic_orbit (m_anomaly, a, e):
    ...
    fac =  sqrt(1-e*e)
    rho = 1.0 - e*cos_E
    # True Anomaly is calculated from the Eccentric anomaly
    f = np.arccos((cos_E-e)/rho)
    if PI <= m_anomaly < TWOPI:
        f = TWOPI - f
    #  The modulus of the position vector is calculated 
    r = a*(1-e*e)/(1+e*np.cos(f))
    # We have the polar coordinates w.r.t the Perifocal Frame,
    # we move to cartesian in the Perifocal Frame, the z coordinate is 0 because
    # we are in the Perifocal Frame
    r_xyz = np.array([r*np.cos(f), r*np.sin(f), 0.0]) 
    # To calculate the velocity we use the formula in Orbital Mechanics for 
    ...
    r_xyz1 =  np.array([a*(cos_E-e), a*fac*sin_E, 0.0]) #x,y at pag 62
    v_xyz1 =  np.array([-cte*sin_E/rho, cte*fac*cos_E/rho, 0.0])

    print (f"cos True Anomaly: {(cos_E-e)/rho}")
    print (f"True Anomaly: {f} rads  {np.rad2deg(f)} degrees ")
    print (f"Mean Anomaly: {m_anomaly} rads  {np.rad2deg(m_anomaly)} degrees ")
    print (f"Old: {r_xyz1}  {v_xyz1}  {np.cross(r_xyz1,v_xyz1)}")
    print (f"NEW: {r_xyz}  {rdot_xyz}  {np.cross(r_xyz,rdot_xyz)}")
    #print (np.isclose(r_xyz1,r_xyz))
    #print (np.isclose(v_xyz1,rdot_xyz))

    # True Anomaly is calculated (Celestial Calculations: A gentle Introduction eq. 4.5.9)
    #f = np.arccos((cos_E-e)/(rho))
    # Angular momentum is calculated
    #h = np.sqrt(GM*a*(1-e*e))
    # Energy
    #Energy = -GM/(2*a)
    #velocity = np.sqrt(2*(Energy+(GM/np.linalg.norm(r))))
    #print (f"Velocity: {velocity},  Modulus velocity :{np.linalg.norm(v)}")
    ...
    return r_xyz, rdot_xyz 
    #return  r_xyz1, v_xyz1
